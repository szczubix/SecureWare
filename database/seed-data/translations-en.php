...
return [
    'services' => [
        ...
        'siem-as-a-service' => [
            ...
            'short_description' => 'We collect and analyze logs from your backup infrastructure and key systems to catch an attack in progress before your data ever gets encrypted.',
            'content' => '<p>Backup is your last line of defense - but the earlier you catch an attack, the less damage it does before it ever reaches your backups. SIEM as a Service is a managed log collection and correlation service for your infrastructure (including your backup environment) that flags suspicious activity - repository access attempts, unusual login patterns, signs of lateral movement - and alerts your team before the attack develops.</p><ul><li>Log collection and correlation across servers, backup, and key systems</li><li>Detection rules tuned to attack patterns targeting backup infrastructure</li><li>Team alerts on suspicious activity</li><li>Monthly event and recommendation report</li></ul><p>We don\'t replace your security team - we add visibility exactly where ransomware attacks most often: the backup layer.</p>',
        ],
    ],
    'pages' => [
        'bezpieczenstwo' => [
            'title' => 'Security',
            'meta_description' => 'How SecureWare protects your backups - encryption, isolation, access control, monitoring and incident response.',
            'content' => '<h2>Security isn\'t an add-on to backup - it\'s the foundation</h2><p>A backup that isn\'t itself secured protects against nothing - ransomware today targets backups just as often as production data. That\'s why every environment we look after is designed assuming an attacker will eventually try to get in there too.</p><h2>Encryption</h2><p>We encrypt data both in transit and at rest. Backups reach the repository over an encrypted channel, and the repositories themselves are encrypted regardless of whether the data lands on our infrastructure or stays with you.</p><h2>Isolation and immutability</h2><p>We store critical copies in repositories that can\'t be overwritten or deleted within the retention window - even with a compromised administrator account. That follows the 3-2-1-1-0 rule, which we treat as the standard, not the exception.</p><h2>Access control</h2><p>We limit access to backup environments to the necessary minimum (least privilege), require multi-factor authentication (MFA), and separate backup permissions from a client\'s production admin permissions - so one compromised account doesn\'t mean losing everything.</p><h2>Monitoring and early detection</h2><p>We monitor the backup environment 24/7, and where a client needs it, we extend that with <a href="/en/oferta/siem-as-a-service">SIEM as a Service</a> - log collection and correlation that flags suspicious activity before data ever gets encrypted.</p><h2>Testing and incident response</h2><p>A backup no one has tested is an assumption, not a safeguard - so we regularly run real restore tests. We also keep a written incident response procedure, so if an attack happens, everyone knows exactly what to do instead of improvising under pressure.</p><p>Have questions about what this looks like for your specific case? <a href="/en/kontakt">Let\'s talk</a>.</p>',
        ],
        'o-nas' => [
            'title' => 'About us',
